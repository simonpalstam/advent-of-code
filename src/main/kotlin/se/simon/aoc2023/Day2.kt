package se.simon.aoc2023


fun main() {
    println("Problem 1 result: ${Day1.problem1()}")
    println("Problem 2 result: ${Day1.problem2()}")
}
object Day1 {
    ...
    fun problem1(): Any {
        ...
        val possibleGames = data.lines().mapNotNull {gameString ->
            var gameIsPossible = true
            val (gameIdString, hands) = gameString.split(":")

            val gameId = gameIdString.split(" ")[1].toInt()

            hands.split(";").map {hand ->
                hand.split(",").map {
                    val (_, quantityString, color) = it.split(" ")
                    val quantity = quantityString.toInt()
                    if (quantity > colorMax[color]!!) {
                        gameIsPossible = false
                    }
                }
            }
            if (gameIsPossible) gameId else null
        }
        return possibleGames.sum()
    }

    fun problem2(): Any {
        val powers = data.lines().map { gameString ->
            val (_, handsString) = gameString.split(":")
            val colorCount = mapOf<String, MutableList<Int>>(
                "red" to mutableListOf(),
                "green" to mutableListOf(),
                "blue" to mutableListOf(),
            )
            handsString.split(";").forEach { hand ->
                hand.split(",").forEach {
                    val (_, quantityString, color) = it.split(" ")
                    val quantity = quantityString.toInt()
                    colorCount[color]!!.add(quantity)
                }
            }
            colorCount.map { it.value.max() }.fold(1) { acc: Int, value: Int -> acc * value }
        }
        return powers.sum()
    }
